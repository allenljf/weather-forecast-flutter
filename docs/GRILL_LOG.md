...
| Q30 | 靜態分析嚴格度 | **1(c)** `flutter_lints` + 手選規則 + `strict-casts`、**2** 加 `riverpod_lint`、**3(b)** 產生覆蓋率報告但不卡門檻 |
...
- CI 需多一個 `dart run custom_lint` 步驟。
...
在進入最後一輪之前，AI 翻查了 Q1–Q31 並找出六處已被後續決定推翻或需修正之處：
